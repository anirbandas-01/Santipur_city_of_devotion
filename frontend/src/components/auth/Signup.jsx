...
import { X, Mail, Lock, User, Eye, EyeOff, ArrowLeft, UserPlus, Phone, Building2 } from "lucide-react";
...
export default function Signup() {
  ...
  const [userType, setUserType] = useState(null); // null, 'club', 'personal'
  ...
  const clubTypes = [
    { value: 'durga-puja', label: 'Durga Puja Committee' },
    { value: 'kali-puja', label: 'Kali Puja Committee' },
    { value: 'saraswati-puja', label: 'Saraswati Puja Committee' },
    { value: 'lakshmi-puja', label: 'Lakshmi Puja Committee' },
  ...
  const handleSubmit = async (e) => {
    ...
    if (userType === 'club' && !formData.clubType) {
      showToast('Please select club type', 'error');
      return;
    }
    ...
    try {
      ...
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/users/register`, payload);
  ...
  const handleBack = () => {
    if (userType) {
      setUserType(null); // Go back to type selection
      setFormData({ name: "", email: "", password: "", phone: "", clubType: "" });
    } else {
      navigate(-1);
    }
  };

  // Show user type selector first
  if (!userType) {
      ...
      <div className="relative bg-white rounded-3xl max-w-md w-full shadow-2xl overflow-hidden animate-slideUp">
        ...
        <div className="bg-gradient-to-br from-purple-600 via-pink-600 to-orange-600 p-10 text-center relative">
          <div className="w-20 h-20 bg-white/20 rounded-full flex items-center justify-center mx-auto mb-4 backdrop-blur-sm">
            {userType === 'club' ? <Building2 size={40} className="text-white" /> : <UserPlus size={40} className="text-white" />}
          </div>
          <h2 className="text-3xl font-bold text-white mb-2">
            {userType === 'club' ? 'Register Your Club' : 'Join as Member'}
          </h2>
          <p className="text-white/90 text-sm">
            {userType === 'club' ? 'Create your club account' : 'Create your personal account'}
          </p>
        </div>

        <form onSubmit={handleSubmit} className="p-8 space-y-5 max-h-[600px] overflow-y-auto">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              {userType === 'club' ? 'Club Name' : 'Your Name'} *
            </label>
            <div className="relative">
              ...
              <input
                type="text"
                placeholder={userType === 'club' ? "Enter club name" : "Enter your name"}
                required
                value={formData.name}
            ...
            <>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-pink-500 focus:outline-none transition-colors text-gray-800"
                >
